...
export function CategoriesModal({ type, id }: CategoriesModalProps) {
	const activeModal: ModalType = useModalStore(state => state.activeModal)
	const closeModal = useModalStore(state => state.closeModal)

	const isOpen: boolean = activeModal === 'category'
		...
		return (
			<Dialog open={isOpen} onOpenChange={open => !open && closeModal()}>
				<DialogContent className="sm:max-w-md">
					...
					{type === 'delete' ? <CategoryDeleteForm id={id} /> : <CategoryForm id={id} />}
				</DialogContent>
			</Dialog>
		)
	}
	...
	return (
		<Drawer open={isOpen} onOpenChange={open => !open && closeModal()}>
			<DrawerContent>
				...
				<div className="p-4">{type === 'delete' ? <CategoryDeleteForm id={id} /> : <CategoryForm id={id} />}</div>
			</DrawerContent>
		</Drawer>
	)
}
